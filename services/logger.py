...
def log_info(
    ...
    **fields: Any,
    ...
    logging.info(_build_message(message, company, step, request_id, fields), stacklevel=2)
...
def log_warning(
    ...
    **fields: Any,
    ...
    logging.warning(_build_message(message, company, step, request_id, fields), stacklevel=2)
...
def log_error(
    ...
    **fields: Any,
    ...
    """
    logging.error(_build_message(message, company, step, request_id, fields), exc_info=exc_info, stacklevel=2)
...
def log_debug(
    ...
    **fields: Any,
    ...
    """
    logging.debug(_build_message(message, company, step, request_id, fields), stacklevel=2)
...
def _build_message(
    ...
    if company:
        parts.append("company=%s" % _safe_value(company))

    if step:
        parts.append("step=%s" % _safe_value(step))

    if request_id:
        parts.append("request_id=%s" % _safe_value(request_id))

    for key, value in fields.items():
        parts.append("%s=%s" % (key, _safe_value(value)))
